...
- **`JsonParseException::Column()` counts UTF-8 bytes, not Unicode codepoints.** Since this system's real data is Korean-language (sample/customer names), a malformed file with Korean text before a syntax error reports a column roughly 2-3x too large (each Korean codepoint is 3 UTF-8 bytes). Accepted for this phase since `Column()`'s contract is already only "some column was recorded" (see above), not an exact position guarantee — flagged here so a later phase's error-reporting UI doesn't rediscover this as a surprise if it ever wants codepoint-accurate columns.
- **Numeric literals that are grammatically valid JSON but overflow `double`** (e.g. `"1e400"`) are rejected as malformed input (`JsonParseException`) rather than saturating to infinity or otherwise being silently accepted — `ParseNumber` treats `std::from_chars`'s `std::errc::result_out_of_range` the same as a hard parse failure. This is a deliberate, if implicit, choice: nothing in this system's real quantities (stock counts, order quantities, minute-granularity durations) approaches numeric ranges anywhere near overflow, so rejecting is safer than silently producing `HUGE_VAL`/infinity. Untested either way by this phase's test list; flagged here in case a later phase's data ever needs a different policy.
